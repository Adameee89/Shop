        ...
        <div className="container mx-auto flex flex-col md:flex-row mt-12 items-center gap-8">
          ...
          <div className="w-full md:w-1/2 flex flex-col gap-5">
            ...
            <div className="mt-10 flex gap-5">
              <Link
                to="/cart"
                className="bg-mainBlue text-white px-5 py-2 rounded-lg hover:bg-mainOrange transition-colors 
                                duration-300 ease-in-out flex items-center justify-center mb-[50px]"
                onClick={handleAddCart}
              >
                Add to Cart
              </Link>
              <button
                className="bg-mainBlue text-white px-5 py-2 rounded-lg hover:bg-mainOrange transition-colors duration-300 
                                ease-in-out flex items-center justify-center mb-[50px]"
                onClick={() => {
                  dispatch(updateFavoriteAction(singleProduct));
                  setFavoriteIdIcon(false);
                }}
              >
                {favoriteIdIcon ? <FaHeart size={25} color="red" /> : <FaHeart size={25} />}
              </button>
            </div>
          </div>
        </div>
